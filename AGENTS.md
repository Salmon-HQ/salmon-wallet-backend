...
- `solana-fungible-token-catalog` — `/ft/verified`, `/ft/search` shapes + fungibility filter. Sources: CoinGecko's Solana token list (the `verified` tag means listed there; `coingeckoId` populated; snapshot ≤24 h per CoinGecko's terms) joined with Triton DAS metadata (`token-metadata-service`: symbol/name/decimals/logo, token program, `swappable` = no Token-2022 transfer fee/hook). Search is local over the catalog (exact symbol first); a bare mint address not in the catalog resolves on-chain and comes back unverified. USD prices (`_price`, `_usdBalance`, `_priceChange24h`, swap USD values) come from CoinGecko `simple/token_price/solana` and are absent, never 0, for unlisted mints. `/v1/networks` carries `attribution { text, url }` on `solana-mainnet` (CoinGecko requires it rendered). No code path calls Jupiter; only the history parser keeps Jupiter program ids
