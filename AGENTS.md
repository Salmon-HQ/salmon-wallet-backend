...
- Cross-domain services with no chain affinity (bridge, coingecko, dapp, geo, network-catalog, network-capabilities, scam, trustwallet) live under `src/services/shared`. Their persistence partners live under `src/repositories/shared` and their public response shapes under `src/resources/shared`.
...
- Upstream providers (StealthEX, Jupiter, Blockdaemon, RPC nodes) reject bad caller input with their own 4xx. `src/middlewares/error-handler.js` maps upstream 400/404/422 onto `bad_request`/`not_found`/`unprocessable_entity` and leaves every other upstream status as 500 — an upstream 401/429/5xx is our credentials or the provider being down, which the caller cannot act on. Do not re-map these per controller.
...
- `bridge-aggregator` — StealthEX-backed surface, chain resolution rules, camelCase exchange shape (`payinAddress` = deposit address) + status normalization on `/exchange` and `/transaction`. Runs on StealthEX API v4; the public symbol vocabulary is upstream's `legacy_symbol`, translated to v4's `(symbol, network)` pair inside the service (`stealthex-catalogue`). `/available` offers only destinations on chains `network-capabilities` enables — bitcoin and solana — because bridging to a chain the wallet cannot show loses the user sight of their funds. The upstream status enum is exactly `waiting|confirming|exchanging|sending|verifying|finished|failed|refunded|expired` (StealthEX API reference, `Exchange.status`); `expired` is terminal and an unrecognized value normalizes to `unknown`, never to `inProgress` — the wallet polls anything in progress for 24h while telling the user their funds are on the way. Every endpoint validates its required params locally (400 `missing_parameter` / `invalid_parameter`) rather than letting the provider answer; an unknown exchange id is 404 `exchange_not_found`.
