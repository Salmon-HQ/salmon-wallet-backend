...
- `solana-source-catalog` — program-ID → source-name map, priority bands, source-add convention. The swap-aggregator router and its limit-order program (ids in `src/constants/solana-program-ids.js`) surface as `source: 'AGGREGATOR'` on every reader path, overriding whatever brand label the enrichment provider attached
