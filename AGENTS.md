...
One job per layer. This is what keeps changes local, providers replaceable, and
...
no controller may mention a signed-transaction body field. Every Powerup
is build-only (`GET /powerups/{id}/build`): the wallet's Apple
...
- Upstream providers (CoinGecko, Blockdaemon, RPC nodes) reject bad caller input with their own 4xx. `src/middlewares/error-handler.js` maps upstream 400/404/422 onto `bad_request`/`not_found`/`unprocessable_entity` and leaves every other upstream status as 500 — an upstream 401/429/5xx is our credentials or the provider being down, which the caller cannot act on. Do not re-map these per controller.
...
- `multichain-account-balance` — array-shape balance response with native/token branching for Bitcoin + Solana. Solana: Blockdaemon Universal is primary (6 s budget); on a transport error or upstream 5xx `solana-balance-provider` falls back to the bare RPC (`solana-rpc-balance-provider`, native + Token + Token-2022 aggregated per mint, same item shape). An upstream 4xx propagates and never triggers the fallback; a fallback failure propagates too — never an empty balance. `amount` is always the raw base-unit balance, the unit every transfer, fee and price calculation is denominated in. A Token-2022 mint whose Scaled UI Amount or Interest Bearing extension makes the displayed figure differ carries `uiAmount` beside it — a decimal string truncated at the mint's decimals, matching what the token program's own `AmountToUiAmount` returns — and clients render that instead of `amount / 10^decimals`; mints that scale 1:1 omit the key entirely. Neither provider reports the multiplier (Blockdaemon returns `confirmed_balance` + `decimals` and nothing else), so `token-ui-amount-service` reads it from the mint, after the filters and only for mints the catalog marks `token-2022`, so a wallet of classic SPL tokens makes no extra request. `usdBalance` follows `uiAmount` when present because CoinGecko quotes these mints per displayed unit — scaled amount with scaled price, raw with raw, never crossed. A failed mint read serves the unscaled amount and logs, rather than failing the balance over one mint.
